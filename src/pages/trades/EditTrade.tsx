...
import { Upload, ArrowLeft, Briefcase, Clock, Activity, Target, Crosshair, DollarSign, Image as ImageIcon, ClipboardCheck } from 'lucide-react';
...
export default function EditTrade() {
  ...
  const [initialSetupId, setInitialSetupId] = useState('');
  ...
  useEffect(() => {
    ...
    if (id) {
      api.get(`/trades/${id}`).then(res => {
        ...
        setTradeDate(d.toISOString().split('T')[0]);
        setTradeTime(trade.tradeTime || `${d.getHours().toString().padStart(2, '0')}:${d.getMinutes().toString().padStart(2, '0')}`);

        setPair(trade.pair || '');
        setDirection(trade.direction || 'Long');
        setBias(trade.bias || '');
        setSetupId(trade.setupId || '');
        setInitialSetupId(trade.setupId || '');
        setConfidence(trade.confidence || '');
        setSession(trade.session || '');
        setEntryPrice(trade.entryPrice ? String(trade.entryPrice) : '');
        setLotSize(trade.lotSize ? String(trade.lotSize) : '');
        setRrRatio(trade.rrRatio ? String(trade.rrRatio) : '');
        setRiskAmount(trade.riskAmount ? String(trade.riskAmount) : '');
        setStatus(trade.status || 'Running');
        setProfitAmount(trade.profitAmount ? String(trade.profitAmount) : '');
        setImageUrlBefore(trade.imageUrlBefore || '');
        setPreTradeNote(trade.preTradeNote || '');
        setExecutionNote(trade.executionNote || '');
        setMistakeNote(trade.mistakeNote || '');
        setLessonNote(trade.lessonNote || '');

        // Restore checklist state from saved data
        if (trade.checklistData && typeof trade.checklistData === 'object') {
          setChecklistState(trade.checklistData);
        }
      }).catch(err => {
  ...
  // Reset checklist state when setup changes (but not on initial load)
  useEffect(() => {
    if (initialSetupId && setupId !== initialSetupId) {
      setChecklistState({});
    }
    ...
    <div className="p-8 max-w-[1600px] mx-auto space-y-6">
      <div className="flex items-center space-x-4 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Modify your execution details and update your thoughts.</p>
        </div>
      </div>
            ...
            <div className="bg-card/80 dark:bg-card/90 backdrop-blur-xl p-6 rounded-2xl border shadow-sm dark:shadow-none border-border">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Input type="number" step="0.00001" required value={entryPrice} onChange={(e: any) => setEntryPrice(e.target.value)} className="bg-muted/30 border-border" />
                </div>
                <div className="grid gap-2">
                  <Label className="text-muted-foreground font-semibold">Lot Size</Label>
                  <Input type="number" step="0.01" value={lotSize} onChange={(e: any) => setLotSize(e.target.value)} className="bg-muted/30 border-border" />
                </div>
